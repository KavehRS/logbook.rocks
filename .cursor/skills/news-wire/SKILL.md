...
- AAJ notes also need `aaj_id:` (the queue id, quoted). Hub `/news/` is two columns side by side: right اخبار جدید (wire, newest first), left ترجمه مقالات (AAJ 2026, newest `aaj_id` first). Each column shows ten items and paginates the rest; a new publish rebuild updates the lists. Within a slot, give the **first** remaining note the later timestamp (+2 min) so newest-first teasers keep pair order.

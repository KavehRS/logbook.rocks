...
4. **Whole-site SEO** — run `.cursor/skills/daily-seo-audit/SKILL.md` for the **entire** site (`/`, `/logbook/`, `/news/`, `/articles/`, sitemap, robots, every indexed URL). Stay on this slot’s news-wire branch (do not open a second SEO-only PR). Log in `_seo/daily-log.md` and refresh `_seo/ranking-snapshot.md` (four pillars + per-report rank 1–2). SEO must not undo translation fixes and must not invent climb facts.

...
Do **not** edit the body, title, description, tags, or categories of existing `_news/*.md` or `_logbook/*.md` files.
...
- `https://logbook.rocks/llms.txt` exists, starts with `#`, lists current hubs
- `https://logbook.rocks/robots.txt` still allows Googlebot / Bingbot / OAI-SearchBot / Claude-SearchBot / PerplexityBot
- `https://logbook.rocks/webmcp-catalog.json` and `/sitemap.xml`
...
Ship on `cursor/<descriptive-name>-4b4e` only when files actually changed.

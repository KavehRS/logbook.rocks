...
UA = "logbook-rocks-seo-ai-source-watch/1.0 (+https://logbook.rocks/)"
...
def spawn_agent() -> None:
    ...
    prompt = """You are the SEO + AI-source watcher for https://logbook.rocks (repo KavehRS/logbook.rocks, branch main).
...
3. Obey .cursor/rules/seo-daily-agent.mdc (never edit existing _news/*.md or _logbook/*.md bodies, titles, or descriptions)
...
Never invent climb facts, never duplicate report prose, never keyword-stuff, never commit secrets."""
    ...
    body = {
        "name": "SEO + AI-source watch — logbook.rocks",
        "prompt": {"text": prompt},
        "repos": [{

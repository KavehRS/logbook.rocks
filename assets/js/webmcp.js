(function () {
  ...
  async function register(catalog) {
    ...
    await ctx.registerTool({
      ...
      description:
        'List published mountaineering ascent reports (گزارش صعود) on logbook.rocks. Optional query filters by title or description.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    await ctx.registerTool({
      name: 'list_news',
      description:
        'List published climbing news (اخبار) on logbook.rocks. Optional query filters by title or description.',
      inputSchema: {
        type: 'object',
        properties: {
          query: { type: 'string', description: 'Optional search text in Persian or English' }
    ...
    await ctx.registerTool({
      name: 'get_news_item',
      description: 'Return one published news item (خبر) by its site path, such as /news/2026-08-19-example/.',
      inputSchema: {
        type: 'object',
        properties: {
